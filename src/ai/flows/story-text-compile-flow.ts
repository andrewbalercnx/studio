...
Respond with only the JSON object now.`;
            ...
            const { synopsis } = structuredOutput;
            // Strip stage-direction brackets (e.g. [Happily], [whispering softly]) from the compiled
            // story text. These are TTS/emotion cues occasionally generated by the beat AI; they must
            // not appear in visual page text or be read verbatim by the TTS system.
            const storyText = structuredOutput.storyText
              .replace(/\[[A-Za-z][^\[\]]*\]\s*/g, '')
              .replace(/\s{2,}/g, ' ')
              .trim();
