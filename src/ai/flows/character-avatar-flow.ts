...
export const characterAvatarFlow = ai.defineFlow(
  ...
  async ({ characterId, feedback }) => {
    ...
    if (!dataUrl) {
      ...
      await logAIFlow({ flowName: 'characterAvatarFlow', sessionId: characterId, parentId: character.ownerParentUid, prompt: promptText, response: llmResponse, startTime, modelName, isFailure: true, failureReason });
