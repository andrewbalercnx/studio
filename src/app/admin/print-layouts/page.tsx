...
function PrintLayoutsPanel() {
  ...
  const { data: layouts, loading } = useCollection<PrintLayout>(layoutsQuery);
    ...
    <Card>
      ...
      <CardContent>
        ...
        {!loading && layouts && layouts.length === 0 && (
          <p className="text-sm text-muted-foreground">No print layouts yet. Create one to get started.</p>
        )}
        {!loading && layouts && layouts.length > 0 && (
          <Table>
            <TableHeader>
